...
public:
  ...
  Handler() {}
  virtual ~Handler() {}
  virtual llvm::Value* EmitBuiltin(const clang::CallExpr* E, const clang::ASTContext& Ctx, const clang::CodeGenPluginEntryPoint& CG) const = 0;
  virtual bool CheckBuiltinCall(clang::CallExpr* E, const clang::Sema& S) const { return false; }
  virtual bool Evaluate(const clang::CallExpr* E, const clang::ASTContext& Ctx, clang::APValue& Value) const { return false; }
};
...
public:
  ...
  bool isPluginBuiltinID(unsigned ID) const {
    return ID >= (Builtin::FirstTSBuiltin + TSRecords.size() + AuxTSRecords.size());
  }
  ...
  const Builtin::Handler* getPluginBuiltinHandler(unsigned ID) const { return PluginRecords[ID - Builtin::FirstTSBuiltin - TSRecords.size() - AuxTSRecords.size()]; }
