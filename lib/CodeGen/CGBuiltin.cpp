...
RValue CodeGenFunction::EmitBuiltinExpr(const FunctionDecl *FD,
  ...
  if (Value *V = EmitPluginBuiltinExpr(BuiltinID, E)) {
    CharUnits Alignment = getContext().getAlignOfGlobalVarInChars(getContext().CharTy);
...
Value *CodeGenFunction::EmitPluginBuiltinExpr(unsigned BuiltinID,
  ...
  if (getContext().BuiltinInfo.isPluginBuiltinID(BuiltinID)) {
    assert(getContext().BuiltinInfo.getPluginBuiltinHandler(BuiltinID) && "Missing builtin handler");
    return getContext().BuiltinInfo.getPluginBuiltinHandler(BuiltinID)->EmitBuiltin(E, getContext(), CodeGenPluginEntryPoint(CGM));
  }
